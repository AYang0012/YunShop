...
| HTTP 客户端 | Axios 1.7 |
| 数据库 | MySQL 8.0.45 |
| 测试框架 | pytest 9.0.3 + Selenium 4.44.0 + Allure 2.16（67 条用例，Page Object 模式） |
...
├── tests/                           ← pytest + Selenium + Allure 自动化测试（67条用例）
│   ├── conftest.py / pytest.ini / run_tests.py  ← 入口配置 + 便捷脚本
│   ├── config/config.py             ← 全局配置（URL、账号、超时、常量）
│   ├── utils/                       ← driver_factory + logger + read_json
│   ├── pages/base_page.py           ← Page Object 基类（~25个通用方法）
│   ├── pages/front/ + admin/        ← 19个页面对象（前台12 + 后台7）
...
python run_tests.py smoke        # 仅冒烟测试（~8条核心流程）
...
pytest cases/front/test_login.py # 单个测试文件
pytest -k "login_success"        # 按名称匹配

# 查看 Allure 报告
allure serve reports/allure-results
```

> **注意**: 运行测试前需确保前后端均已启动，且 MySQL 中有测试数据。
...
测试框架采用 **Page Object 模式 + pytest fixture 依赖注入** 三层架构：

- **conftest.py（根）**: 提供 `driver` fixture（function 级别，每个用例独立浏览器实例）、session 级别配置 fixtures（`test_user`/`admin_user`）、失败自动截图 + 浏览器日志附加到 Allure 报告
- **cases/conftest.py**: 将所有 Page Object 注册为 fixture（`login_page`、`cart_page` 等），提供 `logged_in`/`admin_logged_in` 复合 fixture（自动完成登录前置操作）
- **自定义标记**: `smoke`/`regression`、`front`/`admin`、`p0`/`p1`/`p2`、`login_required`

写入测试时的关键模式：
```python
# 不需要登录 — 注入 Page Object + 测试账号
def test_login_success(self, login_page, test_user):
    login_page.open()
    login_page.login(test_user["mobile"], test_user["password"])
    assert login_page.wait_for_url_contains("/", timeout=5)

# 需要登录 — 注入 logged_in fixture（自动完成登录）
@pytest.mark.login_required
def test_cart(self, cart_page, logged_in):
    cart_page.open()
    ...
```

测试数据生成：`data/test_data.py` 提供 `new_mobile()`、`new_email()`、`new_goods_data()` 等函数，每次生成唯一数据避免数据库冲突。JSON 文件（`login_data.json` 等）用于参数化静态测试场景。
