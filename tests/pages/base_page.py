...
class BasePage:
    ...
    def press_tab(self, locator: Tuple[By, str]) -> None:
        """在指定元素上按 Tab 键。"""
        self.find(locator).send_keys(Keys.TAB)
    ...
    def wait_seconds(self, seconds: float = 1.0) -> None:
        """
        固定等待（仅用于极少数显式等待无法处理的场景）。

        优先使用 wait_for_* 方法，不要滥用此方法。
        """
        time.sleep(seconds)
    ...
    def hover_at_offset(self, locator: Tuple[By, str], x: int, y: int) -> None:
        """鼠标移动到元素内指定偏移位置。"""
        el = self.find(locator)
        ActionChains(self.driver).move_to_element_with_offset(el, x, y).perform()
    ...
    def drag_and_drop(
        self, source: Tuple[By, str], target: Tuple[By, str]
    ) -> None:
        """拖拽元素。"""
    ...
    @staticmethod
    def keys_escape() -> Keys:
        return Keys.ESCAPE

    def press_key(self, locator: Tuple[By, str], key: Keys) -> None:
        """向元素发送按键。"""
